...
def get_ticker_price_change(ticker_symbol, timeframe="today"):
    """
            ...
            if response.status_code == 200:
                try:
                    ...
                    if "Global Quote" in quote_data and quote_data["Global Quote"]:
                        ...
                        error = quote_data.get("Information") or quote_data.get("Note")
                ...
                return f"Error fetching quote for {ticker_symbol}: HTTP {response.status_code}"
        except Exception as e:
            print(f"Error fetching quote for {ticker_symbol}: {str(e)}")
            return f"Error fetching quote for {ticker_symbol}: {str(e)}"
        ...
        if response.status_code == 200:
            try:
                ...
                if "Time Series (Daily)" in data:
                    ...
                    error = data.get("Information") or data.get("Note")
            ...
            print(f"Error: API request failed. Status Code: {response.status_code}, Response: {response.text}")
            return f"Error fetching historical data for {ticker_symbol}: HTTP {response.status_code}"
    except Exception as e:
        print(f"Error fetching historical data for {ticker_symbol}: {str(e)}")
        return f"Error fetching historical data for {ticker_symbol}: {str(e)}"

if __name__ == "__main__":
    sample_ticker = "MSFT"
    print("\n--- Price Change 'today' ---")
    change_today = get_ticker_price_change(sample_ticker, "today")
    print(change_today)

    print("\n--- Price Change 'last 7 days' ---")
    change_7_days = get_ticker_price_change(sample_ticker, "last 7 days")
    print(change_7_days)
...
#     sample_ticker = "MSFT"
#     print("\n--- Price Change 'today' ---")
#     change_today = get_price_change(sample_ticker, "today")
#     print(change_today)

#     print("\n--- Price Change 'last 7 days' ---")
#     change_7_days = get_price_change(sample_ticker, "last 7 days")
#     print(change_7_days)

#     print("\n--- Price Change 'last 30 days' ---")
#     change_30_days = get_price_change(sample_ticker, "last 30 days") 
#     print(change_30_days)
    
#     print("\n--- Price Change for potentially problematic ticker ---")
    
    
#     change_other = get_price_change("IBM", "last 5 days")
#     print(change_other)

#     change_error = get_price_change("NONEXISTENTSTICKERXYZ", "last 7 days")
#     print(change_error)